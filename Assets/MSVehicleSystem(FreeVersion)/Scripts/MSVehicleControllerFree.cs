...
public class WheelClassFree
{
    ...
    [Tooltip(
        "If this variable is true, the wheel associated with this index will receive rotation defined by the flywheel.")]
...
public class VehicleWheelsClassFree
{
    [Range(5, 15000)]
    [Tooltip(
        "In this variable you can define the mass that the wheels will have. The script will leave all wheels with the same mass.")]
...
{
    [Tooltip(
        "If this variable is true, the vehicle will start with the engine running. But this only applies if the player starts inside this vehicle.")]
    public bool startOn = true;

    [Range(500, 2000000)]
    [Tooltip(
        "In this variable you must define the mass that the vehicle will have. Common vehicles usually have a mass around 1500")]
    ...
    [Space(10)]
    [Tooltip(
        "In this variable an empty object affiliated to the vehicle should be associated with the center position of the vehicle, perhaps displaced slightly downward, with the intention of representing the center of mass of the vehicle.")]
...
public class StabilizeTurnsClassFree
{
    ...
    [Range(0.1f, 2.0f)]
    [Tooltip(
        "This variable defines how much lateral force the vehicle will receive when the steering wheel is rotated. This helps the vehicle to rotate more realistically.")]
    public float helpToTurn = 0.35f;

    [Range(0.1f, 1.0f)]
    [Tooltip(
        "This variable defines how fast the vehicle will straighten automatically. This occurs naturally in a vehicle when it exits a curve.")]
    public float helpToStraightenOut = 0.1f;

    [Range(0.1f, 5.0f)]
    [Tooltip(
        "This variable defines how much downforce the vehicle will receive. This helps to simulate a more realistic gravity, but should be set up carefully so as not to make some surreal situations.")]
...
{
    [Tooltip(
        "If this variable is checked, the script will automatically place the 'IgnoreRaycast' layer on the player when needed.")]
    public bool setLayers = true;

    [Tooltip(
        "Here you must associate all the cameras that you want to control by this script, associating each one with an index and selecting your preferences.")]
    public CameraTypeClassFree[] cameras;

    [Tooltip(
        "Here you can configure the cameras, deciding their speed of movement, rotation, zoom, among other options.")]
...
{
    [Tooltip(
        "A camera must be associated with this variable. The camera that is associated here, will receive the settings of this index.")]
    ...
    [Range(0.01f, 1.0f)]
    [Tooltip(
        "Here you must adjust the volume that the camera attached to this element can perceive. In this way, each camera can perceive a different volume.")]
...
public class CameraSettingsClassFree
{
    [Range(0.01f, 0.5f)]
    [Tooltip(
        "The near the camera should possess. This parameter will be adjusted automatically depending on the type of camera.")]
...
public class ETSStyleCameraCameraSettingsClassFree
{
    [Range(1, 20)]
    [Tooltip(
        "In this variable you can configure the sensitivity with which the script will perceive the movement of the mouse. This is applied to cameras that interact with mouse movement only.")]
    public float sensibility = 10.0f;

    [Range(0.5f, 3.0f)]
    [Tooltip(
        "The distance the camera will move to the left when the mouse is also shifted to the left. This option applies only to cameras that have the 'ETS_StyleCamera' option selected.")]
...
public class OrbitalCameraSettingsClassFree
{
    [Range(0.01f, 2.0f)]
    [Tooltip(
        "In this variable you can configure the sensitivity with which the script will perceive the movement of the mouse. ")]
    public float sensibility = 0.8f;

    [Range(0.01f, 2.0f)]
    [Tooltip(
        "In this variable, you can configure the speed at which the orbital camera will approach or distance itself from the player when the mouse scrool is used.")]
...
public class TorqueAdjustmentClassFree
{
    [Range(20, 420)]
    [Tooltip(
        "This variable sets the maximum speed that the vehicle can achieve. It must be configured on the KMh unit")]
    ...
    [Range(0.5f, 2.0f)]
    [Tooltip(
        "This variable defines the speed range of each gear. The higher the range, the faster the vehicle goes, however, the torque is relatively lower.")]
    public float speedOfGear = 1.5f;

    [Range(0.5f, 2.0f)]
    [Tooltip(
        "In this variable, you can manually adjust the torque that each gear has. But it is not advisable to change these values.")]
...
public class VehicleSkidMarksClassFree
{
    ...
    [Range(1.0f, 10.0f)]
    [Tooltip(
        "This variable sets the sensitivity of the vehicle to start generating traces of skidding. The more sensitive, the easier to generate the traces.")]
...
{
    [Tooltip(
        "If this variable is true, the script will simulate a force down on the vehicle, leaving jumps more realistic.")]
    public bool extraGravity = true;

    [Range(0.0f, 10.0f)]
    [Tooltip(
        "This variable defines how much force will be added to the vehicle suspension to avoid rotations. This makes the vehicle more rigid and harder to knock over.")]
    public float feelingHeavy = 1.0f;

    [Range(0.00f, 1.0f)]
    [Tooltip(
        "This variable defines the amount of force that will be simulated in the vehicle while it is tilted, the steeper it is, the lower the force applied. Values too high make the vehicle too tight and prevent it from slipping.")]
    public float downForceAngleFactor = 0.2f;

    [Range(0, 1.0f)]
    [Tooltip(
        "This variable defines how much force will be simulated in the vehicle while on flat terrain. Values too high cause the suspension to reach the spring limit.")]
    public float verticalDownForce = 0.8f;

    [Range(0, 3)]
    [Tooltip(
        "This variable defines a minimum force value that will be simulated. The value corresponds to the mass of the vehicle times the value of this variable.")]
...
public class MSVehicleControllerFree : MonoBehaviour
{
    ...
    [Tooltip(
        "In this class you can adjust various settings that allow changing the way the vehicle is controlled, as well as the initial state of some variables, such as the engine and the brake.")]
    ...
    [Tooltip(
        "In this class, you can adjust all preferences in relation to vehicle skid marks, such as color, width, among other options.")]
    public VehicleSkidMarksClassFree _skidMarks;

    [Tooltip(
        "In this variable, the 'SkidMarks' shader must be associated. Otherwise, the vehicle will not generate skid marks.")]
    ...
    void DebugStartErrors()
    {
        ...
        //
        if (!_wheels.rightFrontWheel.wheelCollider || !_wheels.leftFrontWheel.wheelCollider ||
            !_wheels.rightRearWheel.wheelCollider || !_wheels.leftRearWheel.wheelCollider)
        {
            Debug.LogError(
                "The vehicle must have at least the four main wheels associated with its variables, within class '_wheels'.");
            this.transform.gameObject.SetActive(false);
            return;
        }

        if (!_wheels.rightFrontWheel.wheelMesh || !_wheels.leftFrontWheel.wheelMesh ||
            !_wheels.rightRearWheel.wheelMesh || !_wheels.leftRearWheel.wheelMesh)
        {
            Debug.LogError(
                "The meshes of the four main wheels must be associated with their respective variables within the class '_wheels'.");
            this.transform.gameObject.SetActive(false);
            return;
        }
    }
    ...
    void SetValues()
    {
        ...
        {
            beatsOnWheelSoundAUD = GenerateAudioSource("Sound of wheel beats", 10, 0.25f, _sounds.wheelImpactSound,
                false, false, false);
        }

        if (_sounds.skiddingSound.standardSound)
        {
            skiddingSoundAUD = GenerateAudioSource("Sound of skid", 10, 1, _sounds.skiddingSound.standardSound, false,
                false, false);
        }
        ...
        if (_sounds.collisionSounds.Length > 0)
        {
            ...
            {
                beatsSoundAUD = GenerateAudioSource("Sound of beats", 10, _sounds.volumeCollisionSounds,
                    _sounds.collisionSounds[UnityEngine.Random.Range(0, _sounds.collisionSounds.Length)], false, false,
                    false);
            }
        }
    ...
    void SetCameras()
    {
        ...
        {
            distanceOrbitCamera[x] =
                Vector3.Distance(_cameras.cameras[x]._camera.transform.position, transform.position);
            distanceOrbitCamera[x] = Mathf.Clamp(distanceOrbitCamera[x],
                _cameras.cameraSettings.orbitalCamera.minDistance, _cameras.cameraSettings.orbitalCamera.maxDistance);
            if (!_cameras.cameras[x]._camera)
            {
                Debug.LogError("No camera was associated with variable 'CamerasDoVeiculo.cameras [" + x +
                               "]', therefore an orbital camera will be automatically created in its place.");
    ...
    void InputsCameras()
    {
        if (isInsideTheCar && controls.controls.enable_switchingCameras_Input)
        {
            ...
            else if (Input.GetKeyDown(controls.controls.switchingCameras) &&
                     indexCamera >= (_cameras.cameras.Length - 1))
            {
                indexCamera = 0;
                EnableCameras(indexCamera);
            }
    ...
    void CamerasManager()
    {
        ...
        switch (_cameras.cameras[indexCamera].rotationType)
        {
            case CameraTypeClassFree.TipoRotac.StraightStop:
                newRotationCameras = Quaternion.LookRotation(
                    objStraightStopCameras[indexCamera].transform.position -
                    _cameras.cameras[indexCamera]._camera.transform.position, Vector3.up);
                _cameras.cameras[indexCamera]._camera.transform.rotation = Quaternion.Slerp(
                    _cameras.cameras[indexCamera]._camera.transform.rotation, newRotationCameras,
            ...
            case CameraTypeClassFree.TipoRotac.FirstPerson:
                ...
                rotationX = ClampAngle(rotationX, -_cameras.cameraSettings.firstPersonCamera.horizontalAngle,
                    _cameras.cameraSettings.firstPersonCamera.horizontalAngle);
                rotationY = ClampAngle(rotationY, -_cameras.cameraSettings.firstPersonCamera.verticalAngle,
                    _cameras.cameraSettings.firstPersonCamera.verticalAngle);
                ...
                _cameras.cameras[indexCamera]._camera.transform.localRotation = Quaternion.Lerp(
                    _cameras.cameras[indexCamera]._camera.transform.localRotation, newRotationCameras,
                    Time.deltaTime * 10.0f * timeScaleSpeed);*/
                break;
            case CameraTypeClassFree.TipoRotac.FollowPlayer:
                ...
                {
                    _cameras.cameras[indexCamera]._camera.transform.position = Vector3.Lerp(
                        _cameras.cameras[indexCamera]._camera.transform.position,
                        startPositionCameras[indexCamera].transform.position,
                        Time.deltaTime * _cameras.cameraSettings.followPlayerCamera.displacementSpeed);
                }
                else if (Physics.Linecast(transform.position, startPositionCameras[indexCamera].transform.position,
                             out hitCameras))
                {
                    _cameras.cameras[indexCamera]._camera.transform.position = Vector3.Lerp(
                        _cameras.cameras[indexCamera]._camera.transform.position, hitCameras.point,
                        Time.deltaTime * _cameras.cameraSettings.followPlayerCamera.displacementSpeed);
                }
                ...
                {
                    newRotationCameras = Quaternion.LookRotation(
                        transform.position - _cameras.cameras[indexCamera]._camera.transform.position, Vector3.up);
                    _cameras.cameras[indexCamera]._camera.transform.rotation = Quaternion.Slerp(
                        _cameras.cameras[indexCamera]._camera.transform.rotation, newRotationCameras,
                        Time.deltaTime * _cameras.cameraSettings.followPlayerCamera.spinSpeed);
                }

                break;
            case CameraTypeClassFree.TipoRotac.Orbital:
                minDistanceOrbitalCamera = _cameras.cameraSettings.orbitalCamera.minDistance;
                if (_cameras.cameraSettings.orbitalCamera.invertRotationJoystick && (controls.selectControls ==
                        MSSceneControllerFree.ControlTypeFree.mobileButton))
                {
                ...
                else if (Physics.Linecast(transform.position, _cameras.cameras[indexCamera]._camera.transform.position,
                             out hitCameras))
                {
                    distanceOrbitCamera[indexCamera] = Vector3.Distance(transform.position, hitCameras.point);
                    minDistanceOrbitalCamera = Mathf.Clamp((Vector3.Distance(transform.position, hitCameras.point)),
                        (_cameras.cameraSettings.orbitalCamera.minDistance * 0.5f),
                        _cameras.cameraSettings.orbitalCamera.maxDistance);
                }

                //
                /*xOrbit[indexCamera] += camerasMovX *
                                       (_cameras.cameraSettings.orbitalCamera.sensibility *
                                        distanceOrbitCamera[indexCamera]) /
                                       (distanceOrbitCamera[indexCamera] * 0.5f);
                yOrbit[indexCamera] -= camerasMovY * _cameras.cameraSettings.orbitalCamera.sensibility *
                                       (_cameras.cameraSettings.orbitalCamera.speedYAxis * 10.0f);
                ...
                distanceOrbitCamera[indexCamera] = Mathf.Clamp(
                    distanceOrbitCamera[indexCamera] -
                    (camerasMovZ * _cameras.cameraSettings.orbitalCamera.speedScrool * 50.0f), minDistanceOrbitalCamera,
                    _cameras.cameraSettings.orbitalCamera.maxDistance);
                ...
                _cameras.cameras[indexCamera]._camera.transform.rotation = Quaternion.Lerp(currentRotationCameras,
                    newRotationCameras, Time.deltaTime * 5.0f * timeScaleSpeed);
                _cameras.cameras[indexCamera]._camera.transform.position = Vector3.Lerp(currentPositionCameras,
                    positionCameras, Time.deltaTime * 5.0f * timeScaleSpeed);
                */
                break;
            case CameraTypeClassFree.TipoRotac.OrbitalThatFollows:
                ...
                if (orbitalOn)
                {
                    ...
                    else if (Physics.Linecast(transform.position,
                                 _cameras.cameras[indexCamera]._camera.transform.position, out hitCameras))
                    {
                        distanceOrbitCamera[indexCamera] = Vector3.Distance(transform.position, hitCameras.point);
                        minDistanceOrbitalCamera = Mathf.Clamp((Vector3.Distance(transform.position, hitCameras.point)),
                            minDistanceOrbitalCamera * 0.5f, _cameras.cameraSettings.orbitalCamera.maxDistance);
                    }
                    ...
                    distanceOrbitCamera[indexCamera] = Mathf.Clamp(
                        distanceOrbitCamera[indexCamera] -
                        camerasMovZ * (_cameras.cameraSettings.orbitalCamera.speedScrool * 50.0f),
                        minDistanceOrbitalCamera, _cameras.cameraSettings.orbitalCamera.maxDistance);
                    newDistanceCameras = new Vector3(0.0f, 0.0f, -distanceOrbitCamera[indexCamera]);
                    positionCameras = newRotationCameras * newDistanceCameras + transform.position;
                    currentPositionCameras = _cameras.cameras[indexCamera]._camera.transform.position;
                    currentRotationCameras = _cameras.cameras[indexCamera]._camera.transform.rotation;
            ...
            case CameraTypeClassFree.TipoRotac.ETS_StyleCamera:
                ...
                positionCameras = new Vector3(
                    startETSCamerasPosition[indexCamera].x + Mathf.Clamp(
                        rotationXETS / 50 + (_cameras.cameraSettings.ETS_StyleCamera.ETS_CameraShift / 3.0f),
                        -_cameras.cameraSettings.ETS_StyleCamera.ETS_CameraShift, 0),
                    startETSCamerasPosition[indexCamera].y, startETSCamerasPosition[indexCamera].z);
                _cameras.cameras[indexCamera]._camera.transform.localPosition = Vector3.Lerp(
                    _cameras.cameras[indexCamera]._camera.transform.localPosition, positionCameras,
    ...
    IEnumerator ShakeCameras(float shakeValue, bool returnStartPosition)
    {
        ...
        _cameras.cameras[indexCamera]._camera.transform.position = new Vector3(
            _cameras.cameras[indexCamera]._camera.transform.position.x + Random.Range(-shakeValue, shakeValue),
            _cameras.cameras[indexCamera]._camera.transform.position.y + Random.Range(-shakeValue, shakeValue),
            _cameras.cameras[indexCamera]._camera.transform.position.z + Random.Range(-shakeValue, shakeValue));
    ...
    void Update()
    {
        ...
        inclinationFactorForcesDown = Mathf.Clamp(Mathf.Abs(Vector3.Dot(Vector3.up, transform.up)),
            _vehicleSettings._aerodynamics.downForceAngleFactor, 1.0f);

        if (wheelFDIsGrounded || wheelFEIsGrounded || wheelTDIsGrounded || wheelTEIsGrounded)
        {
            downForceTempLerp =
                (ms_Rigidbody.mass * _vehicleSettings._aerodynamics.minDownForceValue +
                 (_vehicleSettings._aerodynamics.verticalDownForce * Mathf.Abs(KMh * 3.0f) *
                  (ms_Rigidbody.mass / 125.0f))) * inclinationFactorForcesDown;
            downForceUpdateRef = Mathf.Lerp(downForceUpdateRef, downForceTempLerp, Time.deltaTime * 2.5f);
        }
        else
        {
            downForceTempLerp = ms_Rigidbody.mass * _vehicleSettings._aerodynamics.minDownForceValue *
                                inclinationFactorForcesDown;
            downForceUpdateRef = Mathf.Lerp(downForceUpdateRef, downForceTempLerp, Time.deltaTime * 2.5f);
        }
        ...
        if (isInsideTheCar)
        {
            if (Input.GetKeyDown(controls.controls.handBrakeInput) && controls.controls.enable_handBrakeInput_Input &&
                Time.timeScale > 0.2f)
            {
    ...
    void FixedUpdate()
    {
        ...
        if (_vehicleSettings._aerodynamics.extraGravity)
        {
            ...
            if (wheelFDIsGrounded && wheelFEIsGrounded && wheelTDIsGrounded && wheelTEIsGrounded)
            {
                gravityValueFixedUpdate = 4.0f * ms_Rigidbody.mass *
                                          Mathf.Clamp((KMh / _vehicleTorque.maxVelocityKMh), 0.05f, 1.0f);
            }
            else
            {
        ...
        currentDownForceVehicle =
            Mathf.Clamp(Mathf.Lerp(currentDownForceVehicle, downForceValueFixedUpdate, Time.deltaTime * 2.0f), 0.1f,
                4.0f);
    ...
    void SetWheelForces(WheelCollider wheelCollider)
    {
        ...
        if (wheelCollider.isGrounded)
        {
            ...
            distanceXForceTemp = ms_Rigidbody.centerOfMass.y -
                                 transform.InverseTransformPoint(wheelCollider.transform.position).y +
                                 wheelCollider.radius +
                                 (1.0f - wheelCollider.suspensionSpring.targetPosition) *
                                 wheelCollider.suspensionDistance;
            lateralForcePointTemp = tempWheelHit.point + wheelCollider.transform.up *
                _vehicleSettings.improveControl.helpToStraightenOut * distanceXForceTemp;
            forwardForceTemp = tempWheelHit.forwardDir * (tireFO.y) * 3.0f;
            lateralForceTemp = tempWheelHit.sidewaysDir * (tireFO.x);
            if (Mathf.Abs(horizontalInput) > 0.1f && wheelCollider.steerAngle != 0.0f &&
                Mathf.Sign(wheelCollider.steerAngle) != Mathf.Sign(tireSL.x))
            {
    ...
    {
        wheelCollider.GetGroundHit(out tempWheelHit);
        return (localVelocityVector.y + (tempWheelHit.sidewaysSlip *
                                         ((Mathf.Abs(verticalInput) + Mathf.Abs(horizontalInput)) / 2.0f) * (-2.0f))) /
               wheelCollider.radius;
    }
    ...
    public Vector2 LocalSurfaceForce(WheelHit wheelHit)
    {
        ...
        forceFactorTempLocalSurface = Mathf.InverseLerp(1.0f, 0.25f,
            (wheelSpeedLocalSurface - Vector3.Project(wheelSpeedLocalSurface, wheelHit.normal)).sqrMagnitude);
    ...
    public void TireSlips(WheelCollider wheelCollider, WheelHit wheelHit)
    {
        ...
        {
            estimatedSprungMass =
                Mathf.Clamp(wheelHit.force / -Physics.gravity.y, 0.0f, wheelCollider.sprungMass) * 0.5f;
            localRigForceTireSlips = (-estimatedSprungMass * localVelocityWheelTireSlips / Time.deltaTime) +
                                     localSurfaceForceDTireSlips;
            ...
            if (wheelCollider.brakeTorque > 10)
            {
                ...
                minSlipYTireSlips = Mathf.Clamp(Mathf.Abs(reverseForce * tireSlipTireSlips.x) / downForceTireSlips,
                    0.0f, wheelMaxBrakeSlip);
            }
            else
            {
                minSlipYTireSlips = Mathf.Min(Mathf.Abs(reverseForce * tireSlipTireSlips.x) / downForceTireSlips,
                    Mathf.Clamp((verticalInput * 2.5f), -2.5f, 1.0f));
                if (reverseForce != 0.0f && minSlipYTireSlips < 0.1f) minSlipYTireSlips = 0.1f;
            }

            if (Mathf.Abs(tireSlipTireSlips.y) < minSlipYTireSlips)
                tireSlipTireSlips.y = minSlipYTireSlips * Mathf.Sign(tireSlipTireSlips.y);
            ...
            tireForceTireSlips.x =
                Mathf.Clamp(localRigForceTireSlips.x, -rawTireForceTireSlips.x, +rawTireForceTireSlips.x);
            if (wheelCollider.brakeTorque > 10)
            {
    ...
    void StabilizeAngularRotation()
    {
        ...
        {
            ms_Rigidbody.angularVelocity = Vector3.Lerp(ms_Rigidbody.angularVelocity,
                new Vector3(ms_Rigidbody.angularVelocity.x, 0, ms_Rigidbody.angularVelocity.z), Time.deltaTime * 2);
        }
    }
    ...
    void StabilizeWheelRPM()
    {
        if (currentGear > 0)
        {
            if (KMh > (_vehicleTorque.maxVelocityGears[currentGear - 1] * _vehicleTorque.speedOfGear) &&
        ...
        {
            if (KMh > (_vehicleTorque.maxVelocityGears[0] * _vehicleTorque.speedOfGear) &&
                Mathf.Abs(verticalInput) < 0.5f)
            {
                if (_wheels.rightFrontWheel.wheelDrive)
                {
    ...
    void StabilizeVehicleRollForces()
    {
        ...
        {
            leftRearForce =
                (-_wheels.leftRearWheel.wheelCollider.transform.InverseTransformPoint(tempWheelHit.point).y -
                 _wheels.leftRearWheel.wheelCollider.radius) / _wheels.leftRearWheel.wheelCollider.suspensionDistance;
        }
        ...
        {
            rightRearForce =
                (-_wheels.rightRearWheel.wheelCollider.transform.InverseTransformPoint(tempWheelHit.point).y -
                 _wheels.rightRearWheel.wheelCollider.radius) / _wheels.rightRearWheel.wheelCollider.suspensionDistance;
        }
        ...
        {
            leftFrontForce =
                (-_wheels.leftFrontWheel.wheelCollider.transform.InverseTransformPoint(tempWheelHit.point).y -
                 _wheels.leftFrontWheel.wheelCollider.radius) / _wheels.leftFrontWheel.wheelCollider.suspensionDistance;
        }
        ...
        {
            rightFrontForce =
                (-_wheels.rightFrontWheel.wheelCollider.transform.InverseTransformPoint(tempWheelHit.point).y -
                 _wheels.rightFrontWheel.wheelCollider.radius) /
                _wheels.rightFrontWheel.wheelCollider.suspensionDistance;
        }

        //APLICAR FORCAS DESCOBERTAS
        roolForce1 = (leftRearForce - rightRearForce) * _vehicleSettings._aerodynamics.feelingHeavy *
                     _vehicleSettings.vehicleMass * inclinationFactorForcesDown;
        roolForce2 = (leftFrontForce - rightFrontForce) * _vehicleSettings._aerodynamics.feelingHeavy *
                     _vehicleSettings.vehicleMass * inclinationFactorForcesDown;
        //rodasTraz
        if (isGround1)
        {
            ms_Rigidbody.AddForceAtPosition(_wheels.leftRearWheel.wheelCollider.transform.up * -roolForce1,
                _wheels.leftRearWheel.wheelCollider.transform.position);
        }

        if (isGround2)
        {
            ms_Rigidbody.AddForceAtPosition(_wheels.rightRearWheel.wheelCollider.transform.up * roolForce1,
                _wheels.rightRearWheel.wheelCollider.transform.position);
        }

        //rodasFrente
        if (isGround3)
        {
            ms_Rigidbody.AddForceAtPosition(_wheels.leftFrontWheel.wheelCollider.transform.up * -roolForce2,
                _wheels.leftFrontWheel.wheelCollider.transform.position);
        }

        if (isGround4)
        {
            ms_Rigidbody.AddForceAtPosition(_wheels.rightFrontWheel.wheelCollider.transform.up * roolForce2,
                _wheels.rightFrontWheel.wheelCollider.transform.position);
        }
    }
    ...
    public AudioSource GenerateAudioSource(string name, float minDistance, float volume, AudioClip audioClip, bool loop,
        bool playNow, bool playAwake)
    {
    ...
    void SkiddingSounds()
    {
        ...
        if (wheelFDIsGrounded)
        {
            ...
            CheckSKidForward(_wheels.rightFrontWheel.wheelColliderRPM, sensibility75kmh,
                _wheels.rightFrontWheel.forwardSkid, sensibilityLowSpeed);
        }
        ...
        if (wheelFEIsGrounded)
        {
            ...
            CheckSKidForward(_wheels.leftFrontWheel.wheelColliderRPM, sensibility75kmh,
                _wheels.leftFrontWheel.forwardSkid, sensibilityLowSpeed);
        }
        ...
        if (wheelTDIsGrounded)
        {
            ...
            CheckSKidForward(_wheels.rightRearWheel.wheelColliderRPM, sensibility75kmh,
                _wheels.rightRearWheel.forwardSkid, sensibilityLowSpeed);
        }
        ...
        if (wheelTEIsGrounded)
        {
            ...
            CheckSKidForward(_wheels.leftRearWheel.wheelColliderRPM, sensibility75kmh,
                _wheels.leftRearWheel.forwardSkid, sensibilityLowSpeed);
        }
        ...
        if (skiddingIsTrue)
        {
            skiddingSoundAUD.volume = Mathf.Lerp(skiddingSoundAUD.volume,
                (maxSlipTemp * _sounds.skiddingSound.standardVolume), Time.deltaTime * 7.0f);
            if (forwardTempSKid)
            {
                if (skiddingSoundAUD.volume < (0.3f * _sounds.skiddingSound.standardVolume))
                {
                    skiddingSoundAUD.volume = Mathf.Lerp(skiddingSoundAUD.volume,
                        (0.3f * _sounds.skiddingSound.standardVolume), 7.0f * Time.deltaTime);
                }
            }
    ...
    void Sounds()
    {
        ...
        {
            velxCurrentRPM = (Mathf.Clamp(KMh, (_vehicleTorque.minVelocityGears[0] * _vehicleTorque.speedOfGear),
                (_vehicleTorque.maxVelocityGears[0] * _vehicleTorque.speedOfGear)));
            pitchAUD = Mathf.Clamp(
                ((velxCurrentRPM / (_vehicleTorque.maxVelocityGears[0] * _vehicleTorque.speedOfGear)) *
                 _sounds.speedOfEngineSound * engineSoundFactor), 0.85f, _sounds.speedOfEngineSound);
        }
        else
        {
            velxCurrentRPM = (Mathf.Clamp(KMh,
                (_vehicleTorque.minVelocityGears[currentGear - 1] * _vehicleTorque.speedOfGear),
                (_vehicleTorque.maxVelocityGears[currentGear - 1] * _vehicleTorque.speedOfGear)));
            nextPitchAUD =
                ((velxCurrentRPM / (_vehicleTorque.maxVelocityGears[currentGear - 1] * _vehicleTorque.speedOfGear)) *
                 _sounds.speedOfEngineSound * engineSoundFactor);
        ...
        if (_sounds.engineSound)
        {
            if (theEngineIsRunning)
            {
                engineSoundAUD.volume = Mathf.Lerp(engineSoundAUD.volume,
                    Mathf.Clamp(Mathf.Abs(engineInput), 0.35f, 0.85f), Time.deltaTime * 5.0f);
                if (handBrakeTrue || currentGear == 0)
                {
                    engineSoundAUD.pitch = Mathf.Lerp(engineSoundAUD.pitch,
                        0.85f + Mathf.Abs(verticalInput) * (_sounds.speedOfEngineSound * 0.7f - 0.85f),
                        Time.deltaTime * 5.0f);
                }
                else
                {
        ...
        if (_sounds.wheelImpactSound)
        {
            if (Mathf.Abs(lastRightForwardPositionY - _wheels.rightFrontWheel.wheelMesh.transform.localPosition.y) >
                sensImpactFR)
            {
                beatsOnWheelSoundAUD.PlayOneShot(beatsOnWheelSoundAUD.clip);
            }

            if (Mathf.Abs(lastLeftForwardPositionY - _wheels.leftFrontWheel.wheelMesh.transform.localPosition.y) >
                sensImpactFL)
            {
                beatsOnWheelSoundAUD.PlayOneShot(beatsOnWheelSoundAUD.clip);
            }

            if (Mathf.Abs(lastRightRearPositionY - _wheels.rightRearWheel.wheelMesh.transform.localPosition.y) >
                sensImpactRR)
            {
                beatsOnWheelSoundAUD.PlayOneShot(beatsOnWheelSoundAUD.clip);
            }

            if (Mathf.Abs(lastLeftRearPositionY - _wheels.leftRearWheel.wheelMesh.transform.localPosition.y) >
                sensImpactRL)
            {
                beatsOnWheelSoundAUD.PlayOneShot(beatsOnWheelSoundAUD.clip);
            }
    ...
    void OnCollisionEnter(Collision collision)
    {
        if (collision.contacts.Length > 0)
        {
            if (collision.relativeVelocity.magnitude > 5 &&
                collision.contacts[0].thisCollider.gameObject.transform != transform.parent)
            {
                if (_sounds.collisionSounds.Length > 0)
                {
                    beatsSoundAUD.clip =
                        _sounds.collisionSounds[UnityEngine.Random.Range(0, _sounds.collisionSounds.Length)];
    ...
    void AutomaticGears()
    {
        ...
        if (currentGear > 0)
        {
            if (KMh > (_vehicleTorque.idealVelocityGears[currentGear - 1] * _vehicleTorque.speedOfGear +
                       7 * _vehicleTorque.speedOfGear))
            {
            ...
            else if (KMh < (_vehicleTorque.idealVelocityGears[currentGear - 1] * _vehicleTorque.speedOfGear -
                            15 * _vehicleTorque.speedOfGear))
            {
            ...
            if (verticalInput > 0.1f &&
                KMh > (_vehicleTorque.idealVelocityGears[currentGear - 1] * _vehicleTorque.speedOfGear +
                       7 * _vehicleTorque.speedOfGear))
            {
    ...
    void Volant()
    {
        ...
        if (_vehicleSettings.volant)
        {
            if (_vehicleSettings.volant)
            {
                _vehicleSettings.volant.transform.localEulerAngles = new Vector3(
                    _vehicleSettings.volant.transform.localEulerAngles.x,
                    _vehicleSettings.volant.transform.localEulerAngles.y,
                    volantStartRotation + (angle2Volant * 320.0f));
            }
        }
    }
    ...
    public float VehicleTorque(WheelCollider wheelCollider)
    {
        ...
        if ((rpmTempTorque * wheelCollider.radius) >
            (50.0f * _vehicleTorque.numberOfGears * _vehicleTorque.speedOfGear))
        {
            return 0;
        }
        ...
        if (currentGear < 0)
        {
            ...
            torqueM = (500.0f * _vehicleTorque.engineTorque) * clampInputTorque *
                      (_vehicleTorque.gears[0].Evaluate((KMh / _vehicleTorque.speedOfGear))) * -0.8f;
        }
        ...
        {
            torqueM = (500.0f * _vehicleTorque.engineTorque) * (Mathf.Clamp(engineInput, 0f, 1f)) *
                      _vehicleTorque.gears[currentGear - 1].Evaluate((KMh / _vehicleTorque.speedOfGear));
        }
    ...
    {
        leftDifferential =
            1 + Mathf.Abs((0.2f * Mathf.Abs(Mathf.Clamp(horizontalInput, 0, 1))) * (angleRefVolant / 60));
        rightDifferential =
            1 + Mathf.Abs((0.2f * Mathf.Abs(Mathf.Clamp(horizontalInput, -1, 0))) * (angleRefVolant / 60));
        //torque do motor
        if (theEngineIsRunning && isInsideTheCar)
        {
            if (_wheels.rightFrontWheel.wheelDrive)
            {
                _wheels.rightFrontWheel.wheelCollider.motorTorque =
                    VehicleTorque(_wheels.rightFrontWheel.wheelCollider) * rightDifferential;
            }

            if (_wheels.leftFrontWheel.wheelDrive)
            {
                _wheels.leftFrontWheel.wheelCollider.motorTorque =
                    VehicleTorque(_wheels.leftFrontWheel.wheelCollider) * leftDifferential;
            }

            if (_wheels.rightRearWheel.wheelDrive)
            {
                _wheels.rightRearWheel.wheelCollider.motorTorque =
                    VehicleTorque(_wheels.rightRearWheel.wheelCollider) * rightDifferential;
            }

            if (_wheels.leftRearWheel.wheelDrive)
            {
                _wheels.leftRearWheel.wheelCollider.motorTorque =
                    VehicleTorque(_wheels.leftRearWheel.wheelCollider) * leftDifferential;
            }
        }
        else
        {
    ...
    void Brakes()
    {
        ...
        if (isInsideTheCar)
        {
            if (Mathf.Abs(mediumRPM) < 15 && Mathf.Abs(brakeVerticalInput) < 0.05f && !handBrakeTrue &&
                (totalFootBrake + totalHandBrake) < 100)
            {
                brakingAuto = true;
    ...
    void ApplyBrakeInWheels(WheelCollider wheelCollider, bool handBrake)
    {
        ...
        if (!wheelCollider.isGrounded && Mathf.Abs(wheelCollider.rpm) > 0.5f && Mathf.Abs(verticalInput) < 0.05f &&
            wheelCollider.motorTorque < 5.0f)
        {
        ...
        if (KMh < 0.5f && Mathf.Abs(verticalInput) < 0.05f)
        {
            if (wheelCollider.rpm > (25 / wheelCollider.radius))
            {
                wheelCollider.brakeTorque +=
                    0.5f * _vehicleSettings.vehicleMass * Mathf.Abs(wheelCollider.rpm) * Time.deltaTime;
            }
        }
    }
    ...
    void CheckGroundForSKidMarks()
    {
        if (_wheels.rightFrontWheel.wheelCollider)
        {
            if (wheelFDIsGrounded)
            {
                _wheels.rightFrontWheel.generateSkidBool = GenerateSkidMarks(_wheels.rightFrontWheel.wheelCollider,
                    _wheels.rightFrontWheel.wheelWorldPosition,
                    _wheels.rightFrontWheel.rendSKDmarks, _wheels.rightFrontWheel.generateSkidBool,
                    _wheels.rightFrontWheel.skidMarkShift, 0);
            }
            else
            {
        ...
        if (_wheels.leftFrontWheel.wheelCollider)
        {
            if (wheelFEIsGrounded)
            {
                _wheels.leftFrontWheel.generateSkidBool = GenerateSkidMarks(_wheels.leftFrontWheel.wheelCollider,
                    _wheels.leftFrontWheel.wheelWorldPosition,
                    _wheels.leftFrontWheel.rendSKDmarks, _wheels.leftFrontWheel.generateSkidBool,
                    _wheels.leftFrontWheel.skidMarkShift, 1);
            }
            else
            {
        ...
        if (_wheels.rightRearWheel.wheelCollider)
        {
            if (wheelTDIsGrounded)
            {
                _wheels.rightRearWheel.generateSkidBool = GenerateSkidMarks(_wheels.rightRearWheel.wheelCollider,
                    _wheels.rightRearWheel.wheelWorldPosition,
                    _wheels.rightRearWheel.rendSKDmarks, _wheels.rightRearWheel.generateSkidBool,
                    _wheels.rightRearWheel.skidMarkShift, 2);
            }
            else
            {
        ...
        if (_wheels.leftRearWheel.wheelCollider)
        {
            if (wheelTEIsGrounded)
            {
                _wheels.leftRearWheel.generateSkidBool = GenerateSkidMarks(_wheels.leftRearWheel.wheelCollider,
                    _wheels.leftRearWheel.wheelWorldPosition,
                    _wheels.leftRearWheel.rendSKDmarks, _wheels.leftRearWheel.generateSkidBool,
                    _wheels.leftRearWheel.skidMarkShift, 3);
            }
            else
            {
    ...
    private bool GenerateSkidMarks(WheelCollider wheelCollider, Vector3 wheelPos, Mesh wheelMesh, bool generateBool,
        float lateralDisplacement, int indexLastMark)
    {
        ...
        //
        skidTemp = tempWHeelHit.sidewaysDir * (_skidMarks.standardBrandWidth * vehicleScale) / 2f *
                   Vector3.Dot(wheelCollider.attachedRigidbody.velocity.normalized, tempWHeelHit.forwardDir);
        skidTemp -= tempWHeelHit.forwardDir * (_skidMarks.standardBrandWidth * vehicleScale) * 0.1f *
                    Vector3.Dot(wheelCollider.attachedRigidbody.velocity.normalized, tempWHeelHit.sidewaysDir);
        ...
        vertices[GetRepeatedArrayIndex(vertices, verLenght - 1)] = tempWHeelHit.point + tempWHeelHit.normal * 0.02f -
            skidTemp + tempWHeelHit.sidewaysDir * lateralDisplacement;
        vertices[GetRepeatedArrayIndex(vertices, verLenght - 2)] = tempWHeelHit.point + tempWHeelHit.normal * 0.02f +
                                                                   skidTemp + tempWHeelHit.sidewaysDir *
                                                                   lateralDisplacement;
        normals[GetRepeatedArrayIndex(normals, verLenght - 1)] =
            normals[GetRepeatedArrayIndex(normals, verLenght - 2)] = tempWHeelHit.normal;

        Color corRastro = _skidMarks.standardColor;
        corRastro.a = Mathf.Clamp(alphaAplic * 0.9f, 0.01f, 1.0f);

        colors[GetRepeatedArrayIndex(colors, verLenght - 1)] =
            colors[GetRepeatedArrayIndex(colors, verLenght - 2)] = corRastro;
        ...
        tris[GetRepeatedArrayIndex(tris, triLength + 0)] = tris[GetRepeatedArrayIndex(tris, triLength + 3)] =
            tris[GetRepeatedArrayIndex(tris, triLength + 1)] = tris[GetRepeatedArrayIndex(tris, triLength + 4)] =
                tris[GetRepeatedArrayIndex(tris, triLength + 2)] = tris[GetRepeatedArrayIndex(tris, triLength + 5)] =
                    tris[GetRepeatedArrayIndex(tris, triLength + 6)] =
                        tris[GetRepeatedArrayIndex(tris, triLength + 9)] =
                            tris[GetRepeatedArrayIndex(tris, triLength + 7)] =
                                tris[GetRepeatedArrayIndex(tris, triLength + 10)] =
                                    tris[GetRepeatedArrayIndex(tris, triLength + 8)] =
                                        tris[GetRepeatedArrayIndex(tris, triLength + 11)];
    ...
    void SetSkidMarksValues()
    {
        ...
        //
        _wheels.rightFrontWheel.rendSKDmarks = GerarRendRef(skidmarkMaterial,
            _wheels.rightFrontWheel.wheelCollider.gameObject.transform.name);
        _wheels.leftFrontWheel.rendSKDmarks = GerarRendRef(skidmarkMaterial,
            _wheels.leftFrontWheel.wheelCollider.gameObject.transform.name);
        _wheels.rightRearWheel.rendSKDmarks = GerarRendRef(skidmarkMaterial,
            _wheels.rightRearWheel.wheelCollider.gameObject.transform.name);
        _wheels.leftRearWheel.rendSKDmarks = GerarRendRef(skidmarkMaterial,
            _wheels.leftRearWheel.wheelCollider.gameObject.transform.name);
    }
